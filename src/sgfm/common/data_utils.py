...
PYXTAL_SPGLIB_TOL = 0.01
PYXTAL_DEFAULT_TOL = 0.0001
MASKS_NOT_SAME_TOL = 0.0001
...
def normalize_k(data_path: str) -> None:
    ...
    for i,d in tqdm(enumerate(data_dict)):
        mask, _ = c.get_spacegroup_constraint(d['spacegroup'])
        k = torch.from_numpy(d['ks'])
        k = k * mask
        k_arr.append(k)
    k_arr = torch.stack(k_arr, dim=0)
    k_mask = k_arr.abs() > 1e-6
    k_sum = k_arr.sum(dim=0)
    mask_sum = k_mask.sum(dim=0)
    k_mean = k_sum / (mask_sum+ 1e-6)
    k_std= torch.sqrt((k_arr - k_mean.unsqueeze(0) * k_mask).pow(2).sum(dim=0) / (mask_sum+ 1e-6))
    torch.save(k_mean, os.path.join(data_dir, 'k_mean.pt'))
    torch.save(k_std, os.path.join(data_dir, 'k_std.pt'))
    return 

def compose_group_elements(g,h):
    q = torch.zeros_like(g)
    q[:3,:3] = torch.matmul(g[:3,:3],h[:3,:3])
    tmp = torch.matmul(g[:3,:3],h[:3,-1].unsqueeze(-1)).squeeze(-1)
    q[:3,-1] = (tmp+g[:3,-1])%1
    q[-1,-1] = 1
    return q

def construct_inverse(G):
    G_inv = torch.zeros_like(G)
    n = G.shape[0]
    identity = torch.eye(4).float()
    for i in range(n):
        if not torch.allclose(G_inv[i], torch.zeros(4).float()):
            continue
        for j in range(n):
            if torch.allclose(compose_group_elements(G[i],G[j]), identity) and \
                torch.allclose(compose_group_elements(G[j],G[i]), identity):
                G_inv[i] = G[j]
                G_inv[j] = G[i]
                break
    return G_inv

def sample_x0(wyckoff_ops: list[torch.Tensor], dim: int = 3) -> torch.Tensor:
    num_orbits = len(wyckoff_ops)
    x0 = []
    for i in range(num_orbits):
        ops = wyckoff_ops[i]
        z = torch.cat([torch.rand(dim).float(), torch.ones(1).float()], dim=0)
        ops_z = torch.einsum('nij,j->ni', ops, z)
        x0.append(ops_z)
    x0 = torch.cat(x0, dim=0) % 1.0
    return x0[:, :dim]

def sample_xt(data_dict,t,G,frac_coords):
    wyckoff_ops = data_dict['wyckoff_ops']
    num_orbits = len(wyckoff_ops)
    x0 = []
    for i in range(num_orbits):
        ops = wyckoff_ops[i]
        z = torch.cat([torch.rand(3).float(), torch.ones(1).float()], dim=0)
        ops_z = torch.einsum('nij,j->ni', ops, z)
        x0.append(ops_z[:,:-1])
    x0 = torch.cat(x0, dim=0)
    x0 = x0%1
    xt = torus_geodesic_t(x0, frac_coords, t)
    return xt, x0
...
def logmap2(x: torch.Tensor, y: torch.Tensor, wyckoff_ops: list[torch.Tensor]) -> torch.Tensor:
    u = logmap(x, y)
    num_orbits = len(wyckoff_ops)
    ut = []
    counter = 0
    for i in range(num_orbits):
        ops = wyckoff_ops[i]
        z = torch.cat([u[counter,:], torch.zeros(1).float()], dim=0)
        ops_z = torch.einsum('nij,j->ni', ops, z)
        counter += ops.size(0)
        ut.append(ops_z[:,:-1])
...
def get_permutation(frac_coords, G):
    x = torch.Tensor(frac_coords).float()
    x = x-torch.floor(x)
    z = x.unsqueeze(0).repeat(G.size(0),1,1)
    x = torch.cat([x,torch.ones(x.size(0),1).float().to(x.device)],dim=1)
    y = torch.matmul(x.unsqueeze(0), G.transpose(-1,-2))[:,:,:3]
    y = y - torch.floor(y)
    diff_matrix = logmap(z.unsqueeze(1), y.unsqueeze(2))
    diff_norm = torch.norm(diff_matrix, dim=-1)
    matching_indices = diff_norm.argmin(dim=-1)
    # verify its a permutation
    z = matching_indices.sort(dim=-1)
    valid_perm = torch.allclose(z.values, torch.arange(z.values.size(1)).to(z.values.device), atol=1e-5)
    if not valid_perm:
...
def get_symmetry_info(
    ...
    spga = SpacegroupAnalyzer(crystal, symprec=symprec, angle_tolerance=angle_tolerance)
    crystal = spga.get_refined_structure()
    c = pyxtal()
    try:
        c.from_seed(crystal, tol=PYXTAL_SPGLIB_TOL, style='spglib')
    except:
        c.from_seed(crystal, tol=PYXTAL_DEFAULT_TOL)
    ...
    for site in c.atom_sites:
        ...
        for syms in site.wp:
            ...
            tmp_matrices.append(torch.from_numpy(syms.affine_matrix).float())
            coords.append(syms.operate(coord))
            anchors.append(anchor)
        torch_mat.append(torch.stack(tmp_matrices, axis=0))
    anchors = np.array(anchors)
    coords = np.array(coords) % 1.
    G = SpaceGroup(spga.get_space_group_symbol())
    G = torch.stack([torch.from_numpy(g.affine_matrix).float() for g in G.symmetry_ops], dim=0)
    G_inv = construct_inverse(G)
    point_G = G.clone()
    point_G[:,:3,-1] = 0
    # point_G = torch.unique(point_G, dim=0) 
    sym_info = {
        'anchors':anchors,
        'wyckoff_ops':torch_mat,
        'spacegroup':space_group,
        'spacegroup_symbol':spga.get_space_group_symbol(),
        'G': G,
        'G_inv': G_inv,
        'Point_G': point_G,
        }
...
#     # Compute the squared distance between atoms
    ...
    mask_not_same = torch.gt(atom_distance_sqr, MASKS_NOT_SAME_TOL)
...
def process_one(
    ...
    try:
        ...
        if use_space_group:
            crystal, sym_info = get_symmetry_info(crystal, symprec=symprec, angle_tolerance=angle_tolerance)
            result_dict.update(sym_info)
        else:
            result_dict['spacegroup'] = 1
        graph_arrays = build_crystal_graph(crystal, graph_method)
        _, _, lengths, angles, _,_, _ = graph_arrays
        lattice_matrix = lattice_params_to_matrix_torch(torch.from_numpy(lengths).unsqueeze(0), torch.from_numpy(angles).unsqueeze(0))
        lattice_matrix = c_family.de_so3(lattice_matrix)
        ks = c_family.m2v(lattice_matrix).squeeze().numpy()
        G_inv_permutation = get_permutation(graph_arrays[0], result_dict['G_inv'])
        k, n = G_inv_permutation.shape
        G_permutation = torch.empty_like(G_inv_permutation)
        G_permutation[torch.arange(G_inv_permutation.shape[0]).unsqueeze(1), G_inv_permutation] = torch.arange(G_inv_permutation.shape[1])
        result_dict.update({
